...
interface IslandMarkerDef {
  ...
  labelOffset?: [number, number]; // [dx, dy]
  isGeekOnly?: boolean;
}
...
const CARIBBEAN_MARKERS: IslandMarkerDef[] = [
  // Lucayas
  { code: 'BHS', name: 'Bahamas', coords: [-77.39, 25.03], labelOffset: [0, -12] },
  { code: 'TCA', name: 'Turcas y Caicos', coords: [-71.79, 21.69], labelOffset: [0, -10], isGeekOnly: true },

  // Grandes Antillas
  { code: 'CUB', name: 'Cuba', coords: [-79.5, 21.8], labelOffset: [0, -14] },
  { code: 'JAM', name: 'Jamaica', coords: [-77.29, 18.10], labelOffset: [0, 14] },
  { code: 'HTI', name: 'Haití', coords: [-72.28, 18.97], labelOffset: [-16, 0] },
  { code: 'DOM', name: 'Rep. Dominicana', coords: [-70.16, 18.73], labelOffset: [16, 0] },
  { code: 'PRI', name: 'Puerto Rico', coords: [-66.59, 18.22], labelOffset: [0, -12] },
  { code: 'CYM', name: 'Islas Caimán', coords: [-81.25, 19.31], labelOffset: [-14, 0], isGeekOnly: true },

  // Pequeñas Antillas (Arco oriental)
  { code: 'VGB', name: 'Islas Vírgenes (UK)', coords: [-64.63, 18.42], labelOffset: [12, -4], isGeekOnly: true },
  { code: 'VIR', name: 'Islas Vírgenes (EE.UU.)', coords: [-64.89, 18.33], labelOffset: [-12, -4], isGeekOnly: true },
  { code: 'AIA', name: 'Anguila', coords: [-63.06, 18.22], labelOffset: [12, 0], isGeekOnly: true },
  { code: 'SXM', name: 'San Martín', coords: [-63.05, 18.04], labelOffset: [12, 0], isGeekOnly: true },
  { code: 'BLM', name: 'San Bartolomé', coords: [-62.83, 17.90], labelOffset: [12, 0], isGeekOnly: true },
  { code: 'KNA', name: 'San Cristóbal y Nieves', coords: [-62.78, 17.35], labelOffset: [-16, 0] },
  { code: 'ATG', name: 'Antigua y Barbuda', coords: [-61.79, 17.06], labelOffset: [16, 0] },
  { code: 'MSR', name: 'Montserrat', coords: [-62.18, 16.74], labelOffset: [-16, 0], isGeekOnly: true },
  { code: 'GLP', name: 'Guadalupe', coords: [-61.55, 16.26], labelOffset: [16, 0], isGeekOnly: true },
  { code: 'DMA', name: 'Dominica', coords: [-61.37, 15.41], labelOffset: [16, 0] },
  { code: 'MTQ', name: 'Martinica', coords: [-61.02, 14.64], labelOffset: [16, 0], isGeekOnly: true },
  { code: 'LCA', name: 'Santa Lucía', coords: [-60.97, 13.90], labelOffset: [16, 0] },
  { code: 'BRB', name: 'Barbados', coords: [-59.54, 13.19], labelOffset: [16, 0] },
  { code: 'VCT', name: 'San Vicente y Granadinas', coords: [-61.28, 12.98], labelOffset: [-16, 0] },
  { code: 'GRD', name: 'Granada', coords: [-61.68, 12.11], labelOffset: [-16, 0] },
  { code: 'TTO', name: 'Trinidad y Tobago', coords: [-61.22, 10.69], labelOffset: [16, 0] },

  // Antillas del Sur
  { code: 'ABW', name: 'Aruba', coords: [-69.96, 12.52], labelOffset: [-12, -8], isGeekOnly: true },
  { code: 'CUW', name: 'Curazao', coords: [-68.99, 12.16], labelOffset: [0, -10], isGeekOnly: true },
  { code: 'BES', name: 'Bonaire', coords: [-68.23, 12.20], labelOffset: [12, -8], isGeekOnly: true }
];
...
export const CaribbeanInsetMap: React.FC<CaribbeanInsetMapProps> = ({
  ...
  const handleToggleExpand = () => {
    if (!isExpanded) {
      setIsExpanded(true);
      setPosition({ coordinates: [-67.5, 16.2], zoom: 8.5 });
    } else {
      setIsExpanded(false);
    ...
    <>
      ...
      <div className={`relative bg-[#0B1220] border-2 border-cyan-500/60 rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 ${
        ...
        <div className="absolute top-3 left-3 z-10 flex items-center gap-2 bg-slate-900/95 backdrop-blur-md px-3 py-1.5 rounded-xl border border-cyan-500/40 text-xs font-bold text-cyan-300 shadow-xl">
          <span>🏝️</span>
          <span>Caribe & Antillas {isExpanded && '(Vista Gigante Ampliada)'}</span>
        </div>
        ...
        <div className="absolute top-3 right-3 z-10 flex items-center gap-1.5 bg-slate-900/95 backdrop-blur-md p-1.5 rounded-xl border border-slate-700 shadow-xl">
          ...
          <button
            onClick={() => setPosition({ coordinates: isExpanded ? [-67.5, 16.2] : [-70, 16.5], zoom: isExpanded ? 8.5 : 4.8 })}
            className="p-1.5 hover:bg-slate-800 text-slate-300 hover:text-white rounded-lg transition"
        ...
        >
          ...
          >
            ...
            {visibleMarkers.map((island) => {
              ...
              const radius = Math.max(0.35, 1.1 / Math.sqrt(position.zoom));
              const haloRadius = radius * 1.8;
              const hitRadius = Math.max(1.5, 3.2 / Math.sqrt(position.zoom));
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* En vista ampliada, mostrar etiquetas de nombres de las islas */}
                    {isExpanded && (
                      <text
                        x={(island.labelOffset?.[0] || 10) / Math.sqrt(position.zoom / 8.5)}
                        y={(island.labelOffset?.[1] || 0) / Math.sqrt(position.zoom / 8.5)}
